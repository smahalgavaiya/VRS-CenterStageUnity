...
public class DriveReceiverForVirtualMecanumWheel : DriveReceiver
{
    ...
    private void Update()
    {
        ...
        if (wheelVeloc > .01 && power <= 0)
            rb.AddForceAtPosition(-slipDirection.forward * Mathf.Min(powerMultiplier, wheelVeloc * powerMultiplier) * Time.deltaTime * (runWithEncoder ? 2 : 1), transform.position);
